...
CPU::CPU(Memory *_m, Display *_d, Input *_i) : memory(_m), display(_d), input(_i) {
    PC = 0x200;
    SP = 0;
    I = 0;
    d_timer = 0;
...
uint8_t CPU::fetch() { return memory->readByteFromMemory(PC++); }
...
uint16_t OP3XKK(CPU &cpu) {
    if (cpu.R[cpu.X] == cpu.KK) { cpu.PC += 2; }
    return 0;
}
...
uint16_t OP5XY0(CPU &cpu) {
    if (cpu.R[cpu.X] == cpu.R[cpu.Y]) { cpu.PC += 2; }
    return 0;
}
...
uint16_t OPDXYN(CPU &cpu) {
    ...
    for (int row = 0; row < height; row++) {
        ...
        for (int col = 0; col < 8; col++) {
            ...
            if (spritePixel && *screenPixel) { cpu.setVf(); }
...
uint16_t OPEX9E(CPU &cpu) {
    if (cpu.input->isPressed(cpu.R[cpu.X])) { cpu.PC += 2; }
    return 0;
}
...
uint16_t OPFX0A(CPU &cpu) {
    cpu.pause = false;
    while (!cpu.pause) { cpu.input->handleInput(cpu.pause, &cpu.R[cpu.X]); }
    return 0;
}
...
uint16_t OPFX55(CPU &cpu) {
    for (int i = 0; i <= cpu.X; i++) { cpu.memory->writeByteToMemory(cpu.R[i], cpu.I + i); }
    cpu.I += cpu.X + 1;
    return 0;
}

uint16_t OPFX65(CPU &cpu) {
    for (int i = 0; i <= cpu.X; i++) { cpu.R[i] = cpu.memory->readByteFromMemory(cpu.I + i); }
    cpu.I += cpu.X + 1;
    return 0;
}
