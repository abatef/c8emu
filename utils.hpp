...
static inline uint16_t mergeBytes(uint8_t msb, uint8_t lsb) {
    uint16_t _msb = 0x00ff & msb;
    uint16_t _lsb = 0x00ff & lsb;
    return ((_msb << 8) | _lsb);
}

static inline void extractXYN(CPU &cpu, uint8_t msb, uint8_t lsb,
                              uint16_t opcode) {
    cpu.X = msb & 0x0f;
    cpu.Y = lsb >> 4;
...
static inline void updateTimers(CPU &cpu, int ips, int &ins_cnt, Audio &audio) {
    ins_cnt++;
    if (ins_cnt >= ips) {
        if (cpu.d_timer > 0) {
            cpu.d_timer--;
...
static inline void fillJT(Instruction JT[]) {
    for (uint32_t opcode = 0; opcode <= 0xFFFF; ++opcode) {
        ...
        case 0x0000:
            if (nn == 0xE0)
                JT[opcode] = {OP00E0, "CLS", {0, 0, 0, 0}};
            else if (nn == 0xEE)
                JT[opcode] = {OP00EE, "RET", {0, 0, 0, 0}};
            break;
        case 0x1000:
        ...
        case 0x5000:
            if (n == 0x0)
                JT[opcode] = {OP5XY0, "SE Vx, Vy", {0, 0, 0, 0}};
            break;
        case 0x6000:
        ...
        case 0x9000:
            if (n == 0x0)
                JT[opcode] = {OP9XY0, "SNE Vx, Vy", {0, 0, 0, 0}};
            break;
        case 0xA000:
